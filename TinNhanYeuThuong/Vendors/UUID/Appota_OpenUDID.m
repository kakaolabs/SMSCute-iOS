...
// Appota ads offer confirm key
static NSString * const kAppotaAdsConfirmKey = @"AppotaAds_confirmKey";

@interface Appota_OpenUDID (Private)
...
@implementation Appota_OpenUDID
...
//
+ (NSString*) value {
    return [Appota_OpenUDID valueWithError:nil];
}
...
+ (NSString*) getDeviceID {
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0")) {
        ...
        deviceID = [Appota_OpenUDID value];
    }
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [Appota_OpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [Appota_OpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [Appota_OpenUDID _setDict:localDict forPasteboard:slotPB];
    }
...
#pragma mark - Appota ads setting added function
- (void) confirmOpenApp {
